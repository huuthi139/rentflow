    ...
    <>
      ...
      <div className="p-8 space-y-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-primary/60">
          <Link href="/tenants" className="hover:text-primary transition-colors">
            Tenants
          </Link>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-6">
            <div className="size-20 rounded-full bg-primary/10 flex items-center justify-center text-primary text-2xl font-black shrink-0">
              {initials}
            </div>
            <div>
            ...
            <div className="lg:col-span-8 space-y-6">
              ...
              <div className="bg-white rounded-xl border border-primary/10 shadow-sm p-6">
                ...
                <div className="grid grid-cols-2 gap-y-4 gap-x-8">
                  {[
                    ...
                    { label: "ID Number", value: tenant.idNumber },
              ...
              <div className="bg-white rounded-xl border border-primary/10 shadow-sm p-6">
                ...
                <div className="flex gap-4">
                  <div className="size-24 rounded-xl bg-slate-200 flex items-center justify-center shrink-0">
                    <span className="material-symbols-outlined text-3xl text-slate-400">home</span>
                  </div>
                  <div className="space-y-1">
                    <p className="font-bold text-slate-800">{tenant.property}</p>
                    <p className="text-sm text-primary/60">{tenant.propertyType}</p>
                    <p className="text-xs text-primary/50">
                      Contract ends: {tenant.contractEnd}
                    </p>
                    <StatusBadge
                      label={tenant.payment}
                      variant={getStatusVariant(tenant.payment)}
                      dot
                    />
                  </div>
                </div>
                ...
                {tenantPayments.length > 0 ? (
                  <div className="space-y-3">
                    {tenantPayments.slice(0, 4).map((pay) => (
                      ...
                      >
                        <div>
                          <p className="text-sm font-semibold text-slate-800">{pay.month}</p>
                          <p className="text-xs text-primary/50">
                            Due: {pay.dueDate}
                            {pay.paidDate ? ` | Paid: ${pay.paidDate}` : ""}
                          </p>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-sm font-bold">${pay.amount.toLocaleString()}</span>
                          <StatusBadge
                            label={pay.status}
                            variant={getStatusVariant(pay.status)}
                          />
                        </div>
                      </div>
                    ))}
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-white rounded-xl border border-primary/10 shadow-sm p-6">
                ...
                <div className="space-y-4">
                  {[
                    { label: "Months as Tenant", value: "14", icon: "calendar_month" },
                    { label: "Total Paid", value: "$33,600", icon: "account_balance_wallet" },
                    { label: "Current Balance", value: "$0.00", icon: "savings" },
                    ...
                    <div key={stat.label} className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <p className="text-xs text-primary/50 font-medium">{stat.label}</p>
                ...
                {currentContract ? (
                  <div className="space-y-3">
                    {[
                      ...
                      { label: "Monthly Rent", value: `$${currentContract.rentPrice.toLocaleString()}` },
                      { label: "Deposit", value: `$${currentContract.deposit.toLocaleString()}` },
                    ].map((item) => (
                      <div key={item.label} className="flex justify-between">
                        <span className="text-xs text-primary/50 font-medium">{item.label}</span>
                        <span className="text-sm font-bold text-slate-800">{item.value}</span>
                      </div>
                    ))}
                    <div className="flex justify-between items-center pt-2 border-t border-primary/10">
                      <span className="text-xs text-primary/50 font-medium">Status</span>
                      <StatusBadge
                        label={currentContract.status}
                        variant={getStatusVariant(currentContract.status)}
                        dot
                      />
                    </div>
                  </div>
              ...
              <div className="bg-white rounded-xl border border-primary/10 shadow-sm p-6">
                ...
                <div className="space-y-4">
                  {[
                    { text: "Payment received - $2,400", time: "2 days ago", icon: "payments", color: "text-emerald-600" },
                    { text: "Contract renewed for 12 months", time: "1 week ago", icon: "autorenew", color: "text-blue-600" },
                    { text: "Maintenance request submitted", time: "2 weeks ago", icon: "build", color: "text-amber-600" },
                    { text: "Profile information updated", time: "3 weeks ago", icon: "edit", color: "text-slate-600" },
                    { text: "Move-in inspection completed", time: "1 month ago", icon: "check_circle", color: "text-emerald-600" },
                  ].map((activity, i) => (
                    <div key={i} className="flex gap-3">
                      <div className="flex flex-col items-center">
                        ...
                        {i < 4 && <div className="w-px flex-1 bg-primary/10 mt-1" />}
                      </div>
                      <div className="pb-2">
                        <p className="text-sm font-medium text-slate-800">{activity.text}</p>
                        <p className="text-xs text-primary/50">{activity.time}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-primary/10">
                      ...
                      <th className="text-left pb-3 text-xs font-bold uppercase tracking-wider text-primary/50">Property</th>
                      <th className="text-left pb-3 text-xs font-bold uppercase tracking-wider text-primary/50">Start</th>
                      <th className="text-left pb-3 text-xs font-bold uppercase tracking-wider text-primary/50">End</th>
                      <th className="text-left pb-3 text-xs font-bold uppercase tracking-wider text-primary/50">Rent</th>
                  ...
                  <tbody>
                    {tenantContracts.map((c) => (
                      <tr key={c.id} className="border-b border-primary/5 last:border-0">
                        ...
                        <td className="py-3">{c.property}</td>
                        <td className="py-3">{c.startDate}</td>
                        <td className="py-3">{c.endDate}</td>
                        <td className="py-3 font-bold">${c.rentPrice.toLocaleString()}</td>
                        <td className="py-3">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-primary/10">
                      <th className="text-left pb-3 text-xs font-bold uppercase tracking-wider text-primary/50">ID</th>
                      <th className="text-left pb-3 text-xs font-bold uppercase tracking-wider text-primary/50">Month</th>
                      <th className="text-left pb-3 text-xs font-bold uppercase tracking-wider text-primary/50">Due Date</th>
                      <th className="text-left pb-3 text-xs font-bold uppercase tracking-wider text-primary/50">Paid Date</th>
                      <th className="text-left pb-3 text-xs font-bold uppercase tracking-wider text-primary/50">Amount</th>
                      <th className="text-left pb-3 text-xs font-bold uppercase tracking-wider text-primary/50">Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {tenantPayments.map((p) => (
                      <tr key={p.id} className="border-b border-primary/5 last:border-0">
                        ...
                        <td className="py-3">{p.dueDate}</td>
                        <td className="py-3">{p.paidDate || "—"}</td>
                        <td className="py-3 font-bold">${p.amount.toLocaleString()}</td>
                        <td className="py-3">
        ...
        {activeTab === "Documents" && (
          <div className="bg-white rounded-xl border border-primary/10 shadow-sm p-6">
            ...
            <div className="space-y-3">
              {[
                { name: "Lease Agreement - 2024", type: "PDF", date: "Jan 01, 2024", icon: "picture_as_pdf" },
                { name: "ID Verification", type: "PDF", date: "Dec 28, 2023", icon: "badge" },
                { name: "Move-in Inspection Report", type: "PDF", date: "Jan 02, 2024", icon: "fact_check" },
                { name: "Deposit Receipt", type: "PDF", date: "Dec 30, 2023", icon: "receipt" },
              ].map((doc) => (
                <div
                  key={doc.name}
