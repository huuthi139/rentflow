  ...
  return (
    <>
      <Header
        ...
        actions={
          <Link href="/buildings" className="flex items-center gap-2 px-4 py-2 border border-primary/10 rounded-lg text-sm font-medium text-slate-600 hover:bg-primary/5 transition-colors">
      ...
      <div className="p-8 space-y-8">
        ...
        <nav className="flex items-center gap-2 text-sm">
          ...
          <span className="text-slate-600 font-medium">{building.name}</span>
        </nav>
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
              <div className="text-center p-4 bg-slate-50 rounded-xl">
                ...
                <p className="text-xs text-slate-500 uppercase tracking-wider mt-1">Floors</p>
              </div>
              <div className="text-center p-4 bg-slate-50 rounded-xl">
                <p className="text-2xl font-bold text-primary">{building.units}</p>
                <p className="text-xs text-slate-500 uppercase tracking-wider mt-1">Total Units</p>
              </div>
              <div className="text-center p-4 bg-slate-50 rounded-xl">
                <p className="text-2xl font-bold text-primary">${building.serviceFee.toLocaleString()}</p>
                <p className="text-xs text-slate-500 uppercase tracking-wider mt-1">Service Fee/mo</p>
              </div>
              <div className="text-center p-4 bg-slate-50 rounded-xl col-span-2">
                ...
                <p className="text-xs text-slate-500 uppercase tracking-wider mt-1">Owner</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-slate-500">{buildingProperties.length} properties found</span>
          </div>
          ...
          {buildingProperties.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {buildingProperties.map((property) => (
                <Link
                  ...
                  className="bg-white rounded-xl border border-primary/10 shadow-sm overflow-hidden hover:shadow-md transition-shadow group block"
                >
                  <div
                    className="h-40 bg-slate-200 bg-cover bg-center group-hover:scale-105 transition-transform duration-300"
                  ...
                  <div className="p-4 space-y-2">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-xs text-slate-500">Floor {property.floor} | {property.area} sqm</p>
                      </div>
                      <span className={`text-xs font-bold px-2 py-1 rounded-md ${
                        property.status === "Available" ? "bg-emerald-50 text-emerald-600" :
                        property.status === "Occupied" ? "bg-blue-50 text-blue-600" :
                        property.status === "Reserved" ? "bg-amber-50 text-amber-600" :
                        property.status === "Maintenance" ? "bg-red-50 text-red-600" :
            ...
            <div className="bg-white rounded-xl border border-primary/10 shadow-sm p-12 text-center">
              <span className="material-symbols-outlined text-4xl text-slate-300">apartment</span>
              <p className="text-slate-500 mt-2">No properties found in this building.</p>
