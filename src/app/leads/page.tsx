          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              view === "kanban"
                ...
                : "border border-primary/10 text-slate-600 hover:bg-primary/5"
            }`}
          >
        ...
        {view === "kanban" && (
          <div className="grid grid-cols-5 gap-4 min-h-[600px]">
            {stages.map((stage) => {
              ...
              return (
                <div key={stage.key} className="bg-slate-50 rounded-xl flex flex-col min-h-[500px]">
                  {/* Column Header */}
                  <div className={`${stage.color} h-1.5 rounded-t-xl`} />
                  <div className="flex items-center justify-between px-4 py-3">
                    <h3 className="text-sm font-bold text-slate-700">{stage.key}</h3>
                    <span className="bg-white text-xs font-bold text-slate-600 px-2 py-0.5 rounded-full border border-slate-200">
                      {stageLeads.length}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 px-3 pb-3 space-y-3 overflow-y-auto">
                    {stageLeads.map((lead) => (
                      <div
                        key={lead.id}
                        className="bg-white rounded-xl border border-primary/10 shadow-sm p-4 space-y-3 hover:shadow-md transition-shadow cursor-pointer"
                      >
                        {/* Name + Avatar */}
                        <div className="flex items-center gap-3">
                          <div className="size-9 rounded-full bg-primary/15 flex items-center justify-center text-primary text-xs font-bold flex-shrink-0">
                            {getInitials(lead.name)}
                          </div>
                          <div className="min-w-0">
                            <p className="text-sm font-bold text-slate-800 truncate">{lead.name}</p>
                            <p className="text-xs text-slate-500 truncate">{lead.interestedProperty}</p>
                          </div>
                        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-primary/10 bg-slate-50/60">
                    ...
                    <th className="text-left px-6 py-4 font-semibold text-slate-600">Contact</th>
                    <th className="text-left px-6 py-4 font-semibold text-slate-600">Property</th>
                    <th className="text-left px-6 py-4 font-semibold text-slate-600">Budget</th>
                    <th className="text-left px-6 py-4 font-semibold text-slate-600">Stage</th>
                    <th className="text-left px-6 py-4 font-semibold text-slate-600">Source</th>
                    <th className="text-left px-6 py-4 font-semibold text-slate-600">Agent</th>
                ...
                <tbody>
                  {leads.map((lead) => (
                    <tr key={lead.id} className="border-b border-primary/5 hover:bg-primary/[0.02] transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="size-8 rounded-full bg-primary/15 flex items-center justify-center text-primary text-xs font-bold flex-shrink-0">
                            {getInitials(lead.name)}
                          </div>
                      ...
                      <td className="px-6 py-4 text-slate-600">{lead.interestedProperty}</td>
                      <td className="px-6 py-4 font-semibold text-slate-800">{formatCurrency(lead.budget)}</td>
                      <td className="px-6 py-4">
                        <StatusBadge label={lead.stage} variant={stageVariant[lead.stage]} dot />
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-slate-600">{lead.assignedAgent}</td>
