...
const categoryIcons: Record<string, string> = { Plumbing: "water_drop", Electrical: "bolt", HVAC: "ac_unit", Lock: "lock", Internet: "wifi", Furniture: "chair" };
const severityColors: Record<string, string> = { Critical: "bg-red-500", High: "bg-amber-500", Medium: "bg-blue-500", Low: "bg-slate-400" };

export default function IssuesPage() {
  const [activeTab, setActiveTab] = useState("All");
  const filtered = issues.filter((issue) => activeTab === "All" ? true : issue.severity === activeTab);

  return (
    <>
      <Header title="Issue Reports" subtitle="Tenant-reported issues and complaints" actions={<button className="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg text-sm font-bold shadow-lg shadow-primary/20 hover:opacity-90 transition-opacity"><span className="material-symbols-outlined text-lg">add</span>Report Issue</button>} />

      <div className="p-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-primary/10 dark:border-slate-700 shadow-sm"><p className="text-sm text-slate-500 dark:text-slate-400">Open Issues</p><p className="text-2xl font-bold mt-1">12</p></div>
          <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-primary/10 dark:border-slate-700 shadow-sm"><p className="text-sm text-slate-500 dark:text-slate-400">Critical</p><p className="text-2xl font-bold text-red-600 mt-1">3</p></div>
          <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-primary/10 dark:border-slate-700 shadow-sm"><p className="text-sm text-slate-500 dark:text-slate-400">Avg Resolution</p><p className="text-2xl font-bold text-primary mt-1">2.5 days</p></div>
          <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-primary/10 dark:border-slate-700 shadow-sm"><p className="text-sm text-slate-500 dark:text-slate-400">Resolved This Week</p><p className="text-2xl font-bold text-emerald-600 mt-1">8</p></div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {filtered.map((issue) => (
            <div key={issue.id} className="bg-white dark:bg-slate-800 rounded-xl border border-primary/10 dark:border-slate-700 shadow-sm overflow-hidden flex">
              <div className={`w-1 flex-shrink-0 ${severityColors[issue.severity]}`} />
              <div className="flex-1 p-5 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0"><h3 className="text-sm font-bold text-slate-800 dark:text-slate-200">{issue.title}</h3><p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{issue.property}</p></div>
                ...
                <div className="flex items-center gap-4 text-xs text-slate-500 dark:text-slate-400">
                  <span className="inline-flex items-center gap-1"><span className="material-symbols-outlined text-sm text-primary">{categoryIcons[issue.category] || "build"}</span>{issue.category}</span>
                  <span className="inline-flex items-center gap-1"><span className="material-symbols-outlined text-sm">person</span>{issue.reportedBy}</span>
                  <span className="inline-flex items-center gap-1"><span className="material-symbols-outlined text-sm">calendar_today</span>{issue.date}</span>
                </div>
                <div className="flex items-center justify-between pt-1">
                  <div className="flex items-center gap-2"><StatusBadge label={issue.severity} variant={getStatusVariant(issue.severity)} /><StatusBadge label={issue.status} variant={getStatusVariant(issue.status)} dot /></div>
                  {issue.photos > 0 && (<span className="inline-flex items-center gap-1 text-xs text-slate-400"><span className="material-symbols-outlined text-sm">photo_camera</span>{issue.photos}</span>)}
                </div>
              </div>
            </div>
          ))}
