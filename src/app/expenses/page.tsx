...
const categoryVariantMap: Record<string, "primary" | "info" | "warning" | "danger" | "success" | "neutral"> = {
  "Rent to Owner": "primary",
  Electric: "warning",
  Repair: "danger",
  Cleaning: "info",
  Internet: "info",
...
export default function ExpensesPage() {
  ...
  return (
    <>
      <Header
        title="Expenses"
        subtitle="Track property costs and operating expenses"
        actions={
      ...
      <div className="p-8 space-y-6">
        ...
        <div className="bg-white rounded-xl border border-primary/10 shadow-sm overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {filtered.map((e) => (
                <tr key={e.id} className="border-b border-slate-50 hover:bg-primary/[0.02] transition-colors">
                  ...
                  <td className="py-4 px-6">
                    <StatusBadge label={e.category} variant={categoryVariantMap[e.category] || "neutral"} />
                  </td>
                  <td className="py-4 px-6 text-sm text-slate-600">{e.description}</td>
                  <td className="py-4 px-6 text-sm font-bold text-red-600">-${e.amount.toLocaleString()}</td>
                  <td className="py-4 px-6 text-sm text-slate-600">{e.date}</td>
                  <td className="py-4 px-6 text-sm text-slate-600">{e.vendor}</td>
                  <td className="py-4 px-6">
            ...
            <tfoot>
              <tr className="bg-slate-50/80 border-t border-slate-200">
                <td colSpan={4} className="py-4 px-6 text-sm font-bold text-slate-700">Total</td>
                <td className="py-4 px-6 text-sm font-bold text-red-600">-${filteredTotal.toLocaleString()}</td>
                <td colSpan={3}></td>
              </tr>
            </tfoot>
          </table>
