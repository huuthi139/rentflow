...
import MaintenanceForm from "@/components/crud/MaintenanceForm";
...
export default function MaintenancePage() {
  const { maintenance, addMaintenance, updateMaintenance, deleteMaintenance } = useData();
  const { toast } = useToast();
  const [activeTab, setActiveTab] = useState("All Tickets");
  const [modal, setModal] = useState<{ mode: 'add' | 'edit' | 'delete'; ticket?: MaintenanceTicket } | null>(null);

  const filtered = maintenance.filter((t) => activeTab === "All Tickets" ? true : t.status === activeTab);
  const openCount = maintenance.filter(t => t.status === "Open").length;
  const inProgressCount = maintenance.filter(t => t.status === "In Progress").length;
  const totalCost = maintenance.reduce((s, t) => s + t.cost, 0);

  return (
    <>
      <Header title="Maintenance" subtitle="Manage repair and maintenance tickets" actions={
        <div className="flex items-center gap-2">
          <button
            onClick={() => exportToCSV(maintenance.map(t => ({ ID: t.id, Property: t.property, Type: t.type, Description: t.description, Priority: t.priority, Assignee: t.assignee, Cost: t.cost, Status: t.status, 'Report Date': t.reportDate })), 'maintenance')}
      ...
      <div className="p-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-primary/10 dark:border-slate-700 shadow-sm"><p className="text-sm text-slate-500 dark:text-slate-400">Total Tickets</p><p className="text-2xl font-bold mt-1">{maintenance.length}</p></div>
            ...
            <tr key={ticket.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-primary/[0.02] dark:hover:bg-slate-700/50 transition-colors">
              ...
              <td className="py-4 px-6">
                <div className="flex items-center gap-1">
                  <button onClick={() => setModal({ mode: 'edit', ticket })} className="p-1.5 text-slate-400 hover:text-primary hover:bg-primary/5 rounded-lg transition-colors"><span className="material-symbols-outlined text-lg">edit</span></button>
                  <button onClick={() => setModal({ mode: 'delete', ticket })} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"><span className="material-symbols-outlined text-lg">delete</span></button>
                </div>
              </td>
            </tr>
          ))}</tbody></table>
      ...
      <Modal
        ...
        title={modal?.mode === 'edit' ? 'Edit Maintenance Ticket' : 'New Maintenance Ticket'}
        size="lg"
      >
        <MaintenanceForm
          ticket={modal?.ticket}
      ...
      <ConfirmDialog
        ...
        onClose={() => setModal(null)}
        onConfirm={() => { if (modal?.ticket) { deleteMaintenance(modal.ticket.id); toast("Ticket deleted", "success"); } }}
        title="Delete Maintenance Ticket"
        message={`Are you sure you want to delete ticket "${modal?.ticket?.id}"? This action cannot be undone.`}
      />
    </>
  );
}
