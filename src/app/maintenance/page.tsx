...
export default function MaintenancePage() {
  ...
  return (
    <>
      <Header
        title="Maintenance"
        subtitle="Manage repair and maintenance tickets"
        actions={
      ...
      <div className="p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-5 rounded-xl border border-primary/10 shadow-sm">
            ...
            <p className="text-2xl font-bold mt-1">{maintenanceTickets.length}</p>
          </div>
          <div className="bg-white p-5 rounded-xl border border-primary/10 shadow-sm">
            <p className="text-sm text-slate-500">Total Cost</p>
            <p className="text-2xl font-bold text-primary mt-1">${totalCost.toLocaleString()}</p>
          </div>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="text-xs uppercase tracking-wider text-slate-400 border-b border-slate-100 bg-slate-50/50">
                ...
                <th className="text-left py-4 px-6 font-semibold">Assignee</th>
            ...
            <tbody>
              {filtered.map((ticket) => (
                <tr key={ticket.id} className="border-b border-slate-50 hover:bg-primary/[0.02] transition-colors">
                    ...
                    <span className="inline-flex items-center gap-1.5 text-sm">
                      <span className="material-symbols-outlined text-sm text-primary">
                        {ticket.type === "Plumbing" ? "water_drop" :
                         ...
                         ticket.type === "Lock" ? "lock" :
                         ticket.type === "Internet" ? "wifi" :
                         ticket.type === "Appliance" ? "kitchen" :
                         ticket.type === "Furniture" ? "chair" : "build"}
                      </span>
                      {ticket.type}
                    </span>
                  </td>
                  <td className="py-4 px-6 text-sm text-slate-600 max-w-xs truncate">{ticket.description}</td>
                  <td className="py-4 px-6">
                    <StatusBadge label={ticket.priority} variant={getStatusVariant(ticket.priority)} />
                  </td>
                  <td className="py-4 px-6 text-sm text-slate-600">{ticket.assignee}</td>
